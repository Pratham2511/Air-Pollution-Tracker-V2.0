...
const StatBadge = ({ label, value, variant = 'neutral' }) => {
  ...
  return (
    <div className={`flex flex-col gap-1 rounded-xl border px-4 py-3 shadow-sm ${variantClass}`}>
...
const ForecastTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/95 p-3 text-xs text-slate-700 shadow-xl backdrop-blur">
      <p className="font-semibold text-user-primary">Projection • {new Date(label).toLocaleString()}</p>
      <p className="mt-1">
        AQI Projection: <span className="font-semibold text-slate-900">{point?.projectedAqi}</span>
      </p>
    </div>
  );
};
...
const TrendTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/95 p-3 text-xs text-slate-700 shadow-xl backdrop-blur">
      <p className="font-semibold text-user-primary">{new Date(label).toLocaleString()}</p>
      <p className="mt-1">AQI: <span className="font-semibold text-slate-900">{trendPoint?.aqi}</span></p>
      <p className="mt-1 text-slate-500">Rolling Avg: {trendPoint?.rollingAvg}</p>
    </div>
  );
};
...
const AdvisoryCard = ({ advisory }) => (
  <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white/85 p-6 shadow-sm">
    <div className="flex items-center justify-between">
      <span className="rounded-full border border-slate-200 bg-white/90 px-3 py-1 text-xs uppercase tracking-[0.25em] text-slate-500">
        {advisory.severity}
      </span>
      <span className="text-xs text-slate-500">Health Advisory</span>
    </div>
    <div>
      <h3 className="text-lg font-semibold text-slate-900">{advisory.headline}</h3>
    ...
    <ul className="mt-2 flex flex-col gap-2 text-sm text-slate-700">
      {advisory.actions.map((action) => (
        <li key={action} className="flex items-center gap-2">
          <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-slate-200 text-xs text-slate-400 bg-white">
            •
          </span>
          {action}
        </li>
...
const SourceInsight = ({ insight }) => (
  <div className="rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm">
    <div className="flex items-center justify-between">
      <h4 className="text-base font-semibold text-slate-900">{insight.source}</h4>
      <span className="text-xs text-slate-500">
        Confidence {(insight.confidence * 100).toFixed(0)}%
      </span>
    </div>
    <p className="mt-2 text-sm text-slate-600">High impact on {insight.impact}</p>
  </div>
);
...
const CorrelationRow = ({ entry }) => (
  <tr className="border-t border-slate-200">
    <td className="px-4 py-3 text-sm capitalize text-slate-700">{entry.metric}</td>
    <td className="px-4 py-3 text-right text-sm text-slate-600">{entry.correlation}</td>
    <td className="px-4 py-3">
      <div className="relative h-2 w-full overflow-hidden rounded-full bg-slate-100">
        ...
        />
        <div className="absolute left-1/2 top-0 h-full w-[1px] bg-slate-300" />
      </div>
    </td>
  </tr>
);
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-md backdrop-blur">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">AQI Trend & Forecast</h3>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white/75 p-4 shadow-sm">
          <h4 className="text-sm font-semibold text-slate-900">Short-term Outlook</h4>
          <div className="mt-3 flex flex-col gap-2 text-xs text-slate-600">
            {(forecast?.shortTerm ?? []).map((point) => (
              ...
              >
                ...
                <span className="font-semibold text-user-primary">AQI {point.projectedAqi}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white/75 p-4 shadow-sm">
          <h4 className="text-sm font-semibold text-slate-900">Extended Forecast</h4>
          <div className="mt-3 flex flex-col gap-2 text-xs text-slate-600">
            {(forecast?.longTerm ?? []).map((point) => (
              <div
                key={point.timestamp}
              ...
              >
                ...
                <span className="font-semibold text-emerald-600">AQI {point.projectedAqi}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-md backdrop-blur">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p className="text-sm text-slate-600">Relative pollutant load over the selected window.</p>
      </div>
    </div>
        ...
        <AreaChart data={breakdown} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            content={({ active, payload }) => {
              ...
              return (
                <div className="rounded-xl border border-slate-200 bg-white/95 p-3 text-xs text-slate-700 shadow-xl backdrop-blur">
                  <p className="font-semibold text-user-primary">{point.pollutant}</p>
                  <p className="mt-1">
                    Concentration: <span className="font-semibold text-slate-900">{point.value} {point.unit}</span>
                  </p>
                  <p className="mt-1 text-slate-500 capitalize">Role: {point.dominance}</p>
                </div>
              );
            }}
          />
...
const WeatherCorrelationTable = ({ correlations }) => (
  <div className="rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-md backdrop-blur">
    <div className="flex items-center justify-between">
      <div>
        <h3 className="text-lg font-semibold text-slate-900">Weather Correlation</h3>
        <p className="text-sm text-slate-600">How meteorological metrics relate to AQI movement.</p>
      </div>
    </div>
...
const ExposureMetrics = ({ exposure }) => (
  <div className="rounded-3xl border border-slate-200 bg-white/85 p-6 shadow-md backdrop-blur">
    <h3 className="text-lg font-semibold text-slate-900">Exposure Snapshot</h3>
    <p className="mt-1 text-sm text-slate-600">Estimated population impact over the selected window.</p>
    <div className="mt-5 grid gap-4 sm:grid-cols-3">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
        ...
        <p className="mt-2 text-2xl font-semibold text-amber-600">{exposure?.aqiLoadIndex ?? '—'}</p>
      </div>
      <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
        <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Critical Exposure Hours</p>
        <p className="mt-2 text-2xl font-semibold text-rose-600">{exposure?.exposureHours ?? '—'}</p>
      </div>
    </div>
  </div>
);
...
const DominantPollutantCallout = ({ pollutant }) => {
  if (!pollutant) {
    return (
      <div className="rounded-3xl border border-slate-200 bg-white/85 p-6 text-sm text-slate-600 shadow-sm">
  ...
  return (
    <div className="rounded-3xl border border-cyan-200 bg-gradient-to-br from-cyan-50 via-white to-slate-50 p-6 shadow-md">
      <p className="text-xs uppercase tracking-[0.3em] text-user-primary/80">Dominant Pollutant</p>
      <div className="mt-3 flex flex-wrap items-center gap-4">
        <div className="rounded-2xl border border-user-primary/20 bg-user-primary/10 px-4 py-2 text-xl font-semibold text-user-primary">
          {pollutant.pollutant}
        </div>
        <div className="text-sm text-slate-700">
          <p>
            Concentration: <span className="font-semibold text-slate-900">{pollutant.value} {pollutant.unit}</span>
          </p>
          <p className="mt-1 capitalize text-slate-500">Role: {pollutant.dominance}</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-12">
        <header className="flex flex-wrap items-start justify-between gap-6">
          <div className="flex-1 min-w-[16rem]">
            <p className="text-sm uppercase tracking-[0.3em] text-user-primary/80">City Intelligence</p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm">
                  ...
                  <p
                    className={`mt-2 text-3xl font-semibold ${
                      analysis?.comparisons?.delta >= 0 ? 'text-rose-600' : 'text-emerald-600'
                    }`}
                  >
